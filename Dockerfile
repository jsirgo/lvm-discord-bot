...
RUN export WS_PASSWORD=`pwgen -Bs1 12` && \
    ...
    mkdir -p src/config && echo "{\"token\":\""$token"\",\"wsenabled\":"$wsenabled",\"sslenabled\":"$sslenabled",\"tokenkey\":\""${WS_PASSWORD}"\",\"certkey\": \""${CERT_PASSWORD}"\",\"wsusers\":[{\"username\": \""$username"\",\"password\": \""$password"\"}]}" > src/config/config.json && \
